...
function CityTab({ cityId, selectedId }: { cityId: string, selectedId: string | null }) {
    if (cityId === selectedId) return <View
        className={` px-6 h-10 flex flex-row rounded-lg  justify-center items-center`}
    >
...
const data = [
    ...
    return <ScrollView showsHorizontalScrollIndicator={false} horizontal={true} style={{ flexGrow: 0 }} className="rounded-md bg-white p-2 ">
        {data.map((item: string) => {
            return <TouchableOpacity key={item} onPress={() => handlePress(item)}>
